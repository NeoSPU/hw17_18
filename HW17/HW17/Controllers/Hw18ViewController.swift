...
class Hw18ViewController: UIViewController {
    ...
    let oneScreenQueue = DispatchQueue.global()
    let twoScreenQueue = DispatchQueue.global()
    let threeScreenQueue = DispatchQueue.global()
    ...
    private func downloadImageOne(nextIndex: Int) -> UIImage {
        guard index >= 0, index < firstSetArray.count else { return #imageLiteral(resourceName: "placeholder") }
        let imageStringUrl = firstSetArray[index]
    ...
    private func downloadImageTwo(nextIndex: Int) -> UIImage {
        guard index >= 0, index < secondSetArray.count else { return #imageLiteral(resourceName: "placeholder") }
        let imageStringUrl = secondSetArray[index]
        guard let url = URL(string: imageStringUrl) else { return #imageLiteral(resourceName: "placeholder") }
        guard let imageData = try? Data(contentsOf: url) else { return #imageLiteral(resourceName: "placeholder") }
        guard let loadedImage = UIImage(data: imageData) else { return #imageLiteral(resourceName: "placeholder") }
        return loadedImage
    ...
    private func downloadImageThree(nextIndex: Int) -> UIImage {
        guard index >= 0, index < thirdSetArray.count else { return #imageLiteral(resourceName: "placeholder") }
        let imageStringUrl = thirdSetArray[index]
        guard let url = URL(string: imageStringUrl) else { return #imageLiteral(resourceName: "placeholder") }
        guard let imageData = try? Data(contentsOf: url) else { return #imageLiteral(resourceName: "placeholder") }
        guard let loadedImage = UIImage(data: imageData) else { return #imageLiteral(resourceName: "placeholder") }
        return loadedImage
    ...
    @IBAction func ibRefreshButtonPressed(_ sender: Any) {
        ibActivityIndicatorOne.startAnimating()
        oneScreenQueue.async {
            let oneImage = self.downloadImageOne(nextIndex: self.index)
            DispatchQueue.main.async {
                self.ibActivityIndicatorOne.stopAnimating()
                self.ibImageViewOne.image = oneImage
            }
        }
        ibActivityIndicatorTwo.startAnimating()
        twoScreenQueue.async {
            let twoImage = self.downloadImageTwo(nextIndex: self.index)
            DispatchQueue.main.async {
                self.ibActivityIndicatorTwo.stopAnimating()
                self.ibImageViewTwo.image = twoImage
            }
        }
        ibActivityIndicatorThree.startAnimating()
        threeScreenQueue.async {
            let threeImage = self.downloadImageThree(nextIndex: self.index)
            DispatchQueue.main.async {
                self.ibActivityIndicatorThree.stopAnimating()
                self.ibImageViewThree.image = threeImage
            }
        }
